...
function parseSatTLE(lines) {
  ...
  while (i < lines.length) {
    ...
    sats.push({
      ...
      xincl: incl * DEG_TO_RAD,
      xnodeo: raan * DEG_TO_RAD,
      eo: ecc,
      omegao: argp * DEG_TO_RAD,
      xmo: ma * DEG_TO_RAD,
      xno,
      xndt2o: ndot2 * temp,
...
function parseSatCSV(lines) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    sats.push({
      ...
      xincl: (parseFloat(row.INCLINATION) || 0) * DEG_TO_RAD,
      xnodeo: (parseFloat(row.RA_OF_ASC_NODE) || 0) * DEG_TO_RAD,
      eo: parseFloat(row.ECCENTRICITY) || 0,
      omegao: (parseFloat(row.ARG_OF_PERICENTER) || 0) * DEG_TO_RAD,
      xmo: (parseFloat(row.MEAN_ANOMALY) || 0) * DEG_TO_RAD,
      xno,
      xndt2o: ndot * temp,
