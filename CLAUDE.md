...
- `parseAlpha5(s)` — decodes Alpha-5 satellite numbers (A-Z excluding I,O in first position for NORAD IDs 100000–339999).
- `parseSatellites(text)` — auto-detects classic 3-line TLE vs CelesTrak OMM CSV format. TLE parser uses `parseAlpha5()` for satellite numbers.
...
Finds visible satellite passes above a horizon altitude threshold, as seen from a specific location on Earth, over a given Julian Date range.

**`satState(sat, jd, latRad, lonRad)`** — computes topocentric alt/az, eclipse status, and apparent visual magnitude at a given JD. Uses SGP4/SDP4 propagation, WGS84 observer position via `geocentricXYZ()`, Schlyter Sun position converted to equatorial of-date (≈ TEME) for eclipse detection via `inUmbralShadow()`, and `satApparentMag()` for magnitude.

**`findSatellitePasses(sat, jdStart, jdEnd, latRad, lonRad, minAltDeg)`** — main search function. Performs a 1-minute coarse scan detecting altitude threshold crossings, peak altitude, and eclipse transitions. Each event is refined to 1-second precision:
- `refineAltCrossing()` — rise/set times via minimum altitude residual search.
- `refineCulmination()` — peak altitude via maximum altitude search within ±1 coarse step.
- `refineEclipseTransition()` — eclipse entry/exit via status change detection.

**`closePass()`** — finalizes a pass: refines culmination, settles eclipse end state, builds result object with `riseJD`, `riseAz`, `culminJD`, `culminAlt`, `setJD`, `setAz`, `eclipseEntryJD`, `eclipseExitJD`, and `bestMag` (brightest apparent magnitude during the pass).
