...
- **Planet apparent size**: `PLANET_DIAM1AU` in `planets.js` gives equatorial diameter at 1 AU in arcseconds. Angular size is computed on the fly during rendering (`PLANET_DIAM1AU[name] / geoDist / 60` arcminutes); planets are drawn as phase-shaded discs when their pixel radius exceeds the star-dot size. Jupiter and Saturn are drawn as oblate ellipsoids (`PLANET_OBLATENESS`) with Saturn's ring rendered via `drawSaturnRing()`. Pole position angles are computed via stereographic projection Jacobian from J2000 pole directions (`poleJ2k`).
- **Saturn's ring**: Ring plane inclination 28.06° with time-dependent ascending node; tilt `B` = sub-Earth latitude of ring plane. Ring drawn as back half → planet disc → front half (three-layer painter's algorithm). Outer edge at 2.27 Saturn radii, inner edge at 1.24 Saturn radii. Ring hidden when `|sin(B)| < 0.01` (near edge-on).
