...
- **Planet apparent size**: `PLANET_DIAM1AU` in `planets.js` gives equatorial diameter at 1 AU in arcseconds. Angular size is computed on the fly during rendering (`PLANET_DIAM1AU[name] / geoDist / 60` arcminutes); planets are drawn as phase-shaded discs when their pixel radius exceeds the star-dot size.
