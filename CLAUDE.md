...
- **`planets.js`** — Orbital mechanics. `vsop87Position(planet, tau)` evaluates the VSOP87 series for Sun (via Earth) and Mercury–Neptune; Schlyter elements are retained for Pluto only. `moonPositionMeeus(d)` implements Meeus ch.47 truncated ELP2000 lunar theory. Also: Kepler equation solver (elliptical, parabolic, near-parabolic, hyperbolic), magnitude formulas (planets, asteroids via H/G system, comets via H/k), Saturn ring tilt, Moon topocentric parallax (`topocentricCorrection()` with WGS84 ellipsoid), `eclLonJ2000Corr(d)` for Schlyter's precession correction, `asteroidPosition()`/`cometPosition()` from MPC orbital elements, and `helioToGeo()` for heliocentric→geocentric conversion. Day number convention: `d = JD - 2451543.5` (Schlyter's epoch). Asteroid/comet positions are computed in ecliptic of-date (node precessed from J2000) or J2000 ecliptic (node unchanged), depending on the `j2000` flag; when J2000, the Sun's ecliptic longitude is also corrected via `eclLonJ2000Corr` so the geocentric conversion stays in a consistent coordinate system.
